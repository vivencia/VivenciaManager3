...
MainWindow::MainWindow ()
	: QMainWindow ( nullptr ),
	  ...
	  subMenuJobPdfView ( nullptr ), timerUpdate ( nullptr ), sepWin_JobPictures ( nullptr ), sepWin_JobReport ( nullptr ),
...
uint MainWindow::lastViewedRecord ( const uint table ) const
{
	const stringRecord& lvr ( CONFIG ()->readConfigFile ( -1, LAST_VIEWED_RECORDS ) );
...
void MainWindow::setupCalendarMethods ()
{
	...
	ui->lstCalJobsDay->setCallbackForCurrentItemChanged ( [&] ( vmListItem* item ) { return displayJobFromCalendar ( static_cast<dbListItem*>( item ) ); } );
	ui->lstCalJobsWeek->setCallbackForCurrentItemChanged ( [&] ( vmListItem* item ) { return displayJobFromCalendar ( static_cast<dbListItem*>( item ) ); } );
	ui->lstCalJobsMonth->setCallbackForCurrentItemChanged ( [&] ( vmListItem* item ) { return displayJobFromCalendar ( static_cast<dbListItem*>( item ) ); } );
	ui->lstCalPaysDay->setCallbackForCurrentItemChanged( [&] ( vmListItem* item ) { return displayPayFromCalendar ( static_cast<dbListItem*>( item ) ); } );
	ui->lstCalPaysWeek->setCallbackForCurrentItemChanged( [&] ( vmListItem* item ) { return displayPayFromCalendar ( static_cast<dbListItem*>( item ) ); } );
	ui->lstCalPaysMonth->setCallbackForCurrentItemChanged ( [&] ( vmListItem* item ) { return displayPayFromCalendar ( static_cast<dbListItem*>( item ) ); } );
	ui->lstCalBuysDay->setCallbackForCurrentItemChanged ( [&] ( vmListItem* item ) { return displayBuyFromCalendar ( static_cast<dbListItem*>( item ) ); } );
	ui->lstCalBuysWeek->setCallbackForCurrentItemChanged ( [&] ( vmListItem* item ) { return displayBuyFromCalendar ( static_cast<dbListItem*>( item ) ); } );
	ui->lstCalBuysMonth->setCallbackForCurrentItemChanged ( [&] ( vmListItem* item ) { return displayBuyFromCalendar ( static_cast<dbListItem*>( item ) ); } );
}
