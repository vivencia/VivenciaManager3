...
fixDatabase::fixDatabase ()
	: m_badtables ( 5 ), b_needsfixing ( false ), m_pwdMngr ( new passwordSessionManager )
{}
...
bool fixDatabase::checkDatabase ()
{
	...
	QString r_passwd;
	if ( m_pwdMngr->getPassword_UserInteraction ( r_passwd, SYSTEM_ROOT_SERVICE, SYSTEM_ROOT_PASSWORD_ID,
														APP_TR_FUNC ( "In order to perform the check you need to have root privileges. Please type root's password." ) ) )
	{
		...
		if ( readOutputFile ( r_passwd ) )
		{
			...
			m_pwdMngr->savePassword ( passwordSessionManager::SaveSession, r_passwd, SYSTEM_ROOT_SERVICE, SYSTEM_ROOT_PASSWORD_ID );
			return true;
		}
	}
	return false;
