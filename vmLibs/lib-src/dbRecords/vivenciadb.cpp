...
VivenciaDB::VivenciaDB ()
	: m_db ( nullptr ), m_ok ( false ), mNewDB ( false ), mBackupSynced ( true ),
	  mPwdMngr ( new passwordSessionManager ), m_progressMaxSteps_func ( nullptr ), m_errorHandling_func ( nullptr )
{
	if ( openDataBase () )
	{
		// If there are several instances of a database manager, only the first will set that variable for use
		// within DBRecord. For a different outcome, DBRecord::setDatabaseManager has to be called elsewhere later
...
DB_ERROR_CODES VivenciaDB::checkDatabase ( VivenciaDB* vdb )
{
	...
	{
		static_cast<void>( vmNotify::criticalBox ( APP_TR_FUNC ( "Could not connect to mysql server" ),
										 APP_TR_FUNC (	"Make sure the packages mysql-client/server are installed. "
														...
														" or troubleshoot for the problem if it persists." ) ) );
	...
	{			
		...
		if ( !dump.isEmpty () )
		{
			...
			if ( file.open ( QIODevice::WriteOnly | QIODevice::Text ) )
			{
				...
				m_progressStepUp_func ();
				mBackupSynced = true;
				static_cast<void>( mPwdMngr->savePassword ( passwordSessionManager::SavePermanment, passwd, VMDB_PASSWORD_SERVICE, VMDB_PASSWORD_SERVER_ADMIN_ID ) );
				return true;
			}
		}
	}
