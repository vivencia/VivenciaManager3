...
static vmCheckBox* chkPlaySound ( nullptr );
static vmCheckBox* chkExecApp ( nullptr );
static vmCheckBox* chkShowMessage ( nullptr );
...
todoList::todoList ( QWidget* parent )
	: mParent ( parent ), mLstToDo ( nullptr ), mTdbList ( nullptr ),
	  mTxtInfo ( nullptr )
{
	mListFileName = CONFIG ()->appDataDir () + CONFIG ()->loggedUser () + QLatin1String ( "-todolist" );
	...
	mLstToDo->setCallbackForCellNavigation ( [&] ( const uint row, const uint col, const uint prev_row, const uint prev_col ) { return 
			cellActivated ( row, col, prev_row, prev_col ); } );
	...
	chkShowMessage = new vmCheckBox ( tr ( "Show Message" ) );
	chkShowMessage->setEnabled ( false );
	chkShowMessage->setCallbackForContentsAltered ( [&] ( const vmWidget* const ) { return showMessage ( chkShowMessage->isChecked () ); } );
	txtMessage = new QLineEdit;
	txtMessage->setEnabled ( false );

	chkPlaySound = new vmCheckBox ( tr ( "Play sound" ) );
	chkPlaySound->setEnabled ( false );
	chkPlaySound->setCallbackForContentsAltered ( [&] ( const vmWidget* const ) { return playSound ( chkPlaySound->isChecked () ); } );
	btnChooseSound = new QToolButton;
	btnChooseSound->setEnabled ( false );
	txtSound = new QLineEdit;
	txtSound->setEnabled ( false );

	chkExecApp = new vmCheckBox ( tr ( "Run app" ) );
	chkExecApp->setEnabled ( false );
	chkExecApp->setCallbackForContentsAltered ( [&] ( const vmWidget* const ) { return execApp ( chkExecApp->isChecked () ); } );
