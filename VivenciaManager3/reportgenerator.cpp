...
void reportGenerator::btnInsertFooter_clicked ( const int c_id )
{
    ...
    mCursor.insertBlock ();
    mCursor.insertText ( QStringLiteral ( " (Reforma, Pintura, Paisagismo e Jardinagem)" ) );
    mCursor.insertBlock ();
    mCursor.insertText ( QStringLiteral ( "Telefones: (19) 99788-9809 / (19) 3483-2252" ) );
    mCursor.insertBlock ();
    mCursor.insertText ( QStringLiteral ( "São Pedro/SP - CEP 13520-970 Caixa Postal: 174" ) );
    mCursor.insertBlock ();
...
{
    setIgnoreCursorPos ( true );
    TEXT_EDITOR_TOOLBAR ()->insertImage ( CONFIG ()->vivienciaLogo (), QTextFrameFormat::FloatRight );
    if ( c_id == -1 )
        setIgnoreCursorPos ( false );
}
    ...
    if ( recIntValue ( rgClient, FLD_CLIENT_ID ) != -1 ) {
        ...
        if ( rgPay->readFirstRecord ( FLD_PAY_CLIENTID, recStrValue ( rgClient, FLD_CLIENT_ID ) ) ) {
            ...
            /*do {
            	if ( !rgPay->opt ( FLD_PAY_PAID ) ) {
            		if ( rgJob->readRecord ( rgPay->actualRecordStr ( FLD_PAY_JOBID ).toInt () ) ) {
            			...
            			TEXT_EDITOR_TOOLBAR ()->btnBold_clicked ( false );
            			jobs_total += rgPay->price ( FLD_PAY_VALUE );
            		}
            	}
            ...
            if ( !jobs_total.isNull () ) {
                ...
                TEXT_EDITOR_TOOLBAR ()->btnBold_clicked ( true );
                TEXT_EDITOR_TOOLBAR ()->btnItalic_clicked ( true );
                mCursor.insertText ( QString ( QStringLiteral ( "Total dos serviços: %1" ) ).arg ( jobs_total.toPrice () ) );
                mCursor.insertBlock ();
                mCursor.insertBlock ();
