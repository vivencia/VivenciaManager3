...
bool dbSuppliesUI::setupUI ()
{
    ...
    this, [&] ( const bool checked ) {
        return btnSuppliesEditTable_clicked ( checked );
    } );
    ...
    this, [&] ( const bool ) {
        return btnSuppliesCancelEdit_clicked ();
    } );
    ...
    this, [&] ( const bool ) {
        return btnSuppliesInsertRowAbove_clicked ();
    } );
    ...
    this, [&] ( const bool ) {
        return btnSuppliesInsertRowBelow_clicked ();
    } );

    btnSuppliesRemoveRow = new QPushButton ( tr ( "Remove item" ) );
    btnSuppliesRemoveRow->connect ( btnSuppliesRemoveRow, &QPushButton::clicked,
    this, [&] ( const bool ) {
        return btnSuppliesRemoveRow_clicked ();
    } );

    QGridLayout* gLayoutdbSuppliesButtons ( new QGridLayout );
    gLayoutdbSuppliesButtons->addWidget ( btnSuppliesEditTable, 0, 0 );
    gLayoutdbSuppliesButtons->addWidget ( btnSuppliesCancelEdit, 0, 1 );
    gLayoutdbSuppliesButtons->addWidget ( new QWidget (), 0, 3, 1, 1 );
    gLayoutdbSuppliesButtons->addWidget ( btnSuppliesInsertRowAbove, 1, 0 );
    gLayoutdbSuppliesButtons->addWidget ( btnSuppliesInsertRowBelow, 1, 1 );
    gLayoutdbSuppliesButtons->addWidget ( btnSuppliesRemoveRow, 1, 2 );

    createTable ();

    suppliesLayout = new QVBoxLayout;
    m_table->addToLayout ( suppliesLayout );
    suppliesLayout->addLayout ( gLayoutdbSuppliesButtons, 1 );
...
void dbSuppliesUI::btnSuppliesRemoveRow_clicked ()
{
    if ( m_table->currentRow () >= 1 )
        rowRemoved ( m_table->currentRow () );
}

void dbSuppliesUI::btnSuppliesInsertRowBelow_clicked ()
{
    insertRow ( m_table->currentRow () + 1 );
}

void dbSuppliesUI::btnSuppliesInsertRowAbove_clicked ()
{
