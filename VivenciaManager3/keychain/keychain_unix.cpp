	...
	switch ( getKeyringBackend () )
	{
		...
		case Backend_GnomeKeyring:
			...
			if ( !GnomeKeyring::find_network_password ( key.toUtf8().constData(),
					...
					reinterpret_cast<GnomeKeyring::OperationGetStringCallback>( &passwdManagerPrivate::gnomeKeyring_readCb ), this, 0 ) )
	...
	switch ( getKeyringBackend () )
	{
		...
		case Backend_GnomeKeyring:
		{
			...
			if ( !GnomeKeyring::store_network_password ( GnomeKeyring::GNOME_KEYRING_DEFAULT,
				...
				password.constData (), reinterpret_cast<GnomeKeyring::OperationDoneCallback>( &passwdManagerPrivate::gnomeKeyring_writeCb ), this, 0 ) )
					q->emitFinishedWithError( OtherError, tr("Unknown error") );
		}
		break;
