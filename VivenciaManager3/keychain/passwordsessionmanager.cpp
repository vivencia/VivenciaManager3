...
void deletepasswordSessionManagerInstance ()
{
	heap_del ( passwordSessionManager::s_instance );
}
...
bool passwordSessionManager::getPassword_UserInteraction ( QString& passwd, const QString& service, const QString& id, const QString& message )
{
	if ( !getPassword ( passwd, service, id ) )
	{
		...
		{
			if ( VM_NOTIFY ()->passwordBox ( passwd, nullptr, APP_TR_FUNC ( "Password required" ), message ) )
				return true;
		}
	}
	else
