...
machinesDlg* machinesDlg::s_instance ( nullptr );

void deleteMachinesInstance ()
{
	heap_del ( machinesDlg::s_instance );
}
...
void machinesDlg::showJobMachineUse ( const QString& jobid )
{
	...
	if ( !jobid.isEmpty () && jobid != QStringLiteral ( "-1" ) ) {
		if ( mMacRec->action () != ACTION_READ ) {
			if ( VM_NOTIFY ()->questionBox ( tr ( "Save current information before showing the requested logs?" ),
...
{
	QStringList list;
	APP_COMPLETERS ()->fillList ( vmCompleters::MACHINE_EVENT, list );
	cboEvents->addItems ( list );
	APP_COMPLETERS ()->fillList ( vmCompleters::MACHINE_NAME, list );
	cboMachines->addItems ( list );
	APP_COMPLETERS ()->fillList ( vmCompleters::STOCK_TYPE, list );
	cboType->addItems ( list );
	APP_COMPLETERS ()->fillList ( vmCompleters::BRAND, list );
...
int machinesDlg::loadJobMachineEventsTable ( const QString& jobid )
{
	...
	if ( machine_id == -1 )
		VM_NOTIFY ()->notifyMessage ( tr ( "Invalid Job ID" ), tr ( "There were no records associated with this job ID (" ) + jobid + CHR_R_PARENTHESIS );
	return machine_id;
}
...
bool machinesDlg::loadData ( const int id )
{
	...
	if ( mMacRec->action () == ACTION_READ ) {
		clearForms ();
		ok = ( id == -1 ) ? mMacRec->readLastRecord () : mMacRec->readRecord ( id );
		if ( ok ) {
...
void machinesDlg::getSelectedJobID ( const int jobid )
{
	...
	if ( jobid != -1 ) {
		...
		if ( cboEvents->currentIndex () != -1 )
			curData.changeValue ( cboEvents->currentIndex (), strJobID );
		else
		...
		do {
			...
			if ( recValue.contains ( cboMachines->text (), Qt::CaseInsensitive ) ) {
				...
				while ( ( index = machineName.field ( cboMachines->text (), index ) ) >= 0 ) {
					tableMachineHistory->setCellValue ( stringRecord::fieldValue ( recStrValue ( &mrec, FLD_MACHINES_EVENTS_DESC ), index ), row, 0 );
					tableMachineHistory->setCellValue ( stringRecord::fieldValue ( recStrValue ( &mrec, FLD_MACHINES_EVENT_DATES ), index ), row, 1 );
					tableMachineHistory->setCellValue ( stringRecord::fieldValue ( recStrValue ( &mrec, FLD_MACHINES_EVENT_TIMES ), index ), row, 2 );
...
void machinesDlg::dataAltered ( const vmWidget* const sender )
{
	...
	QString data;
	stringRecord curData ( recStrValue ( mMacRec, sender->id () ) );
	...
	if ( eventIndex != -1 )
		curData.changeValue ( eventIndex, data );
	else
		curData.insertField ( 0, data );
	setRecValue ( mMacRec, sender->id (), curData.toString () );
}
...
void machinesDlg::btnDelEvent_clicked ()
{
	...
	if ( field >= 0 ) {
		if ( VM_NOTIFY ()->questionBox ( tr ( "Machines Events" ), tr ( "Confirm removing event " ) + cboEvents->text () + CHR_QUESTION_MARK ) ) {
			strRec.removeField ( field );
			...
			if ( mMacRec->saveRecord () ) {
				tableMachineHistory->removeRow ( field );
				(void) loadData ( -1 );
				VM_NOTIFY ()->notifyMessage ( tr ( "Machines Events" ), tr ( "Event info deleted!" ) );
			}
		}
	}
}
...
void machinesDlg::btnSave_clicked ()
{
	...
	if ( mMacRec->saveRecord () ) {
		...
		row = tableJobEvents->lastUsedRow () + 1;
